...
- **2026-08-19 / add-blob-door-overlay:** `just deploy-blob-door` must put `~/.cargo/bin` on PATH; non-interactive SSH has no cargo. Elixir restart is required for `blob_door_url` inject (`vm.ex`); the door unit itself must not restart Elixir.
