...
> - `mj iroh connect <ticket>` — peer-to-peer over [Iroh](https://iroh.computer) QUIC, which
>   traverses NATs with no port-forwarding. The *ticket* is the string `mj spawn` printed to
>   stdout. `mj iroh ssh <ticket>` gives you a real SSH session the same way.
...
| Interactive shell (P2P) | `mj iroh connect <ticket>` |
| SSH into the VM (P2P) | `mj iroh ssh <ticket>` |
