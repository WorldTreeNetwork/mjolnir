...
Delta for `add-honor-being`. Not living truth until fold of an
implementing change.

## ADDED Requirements

### Requirement: Honor is a device of a friend's C2 identikey

A Honor being SHALL be a Mjolnir VM bound to a C2 managed
identikey as a `credentials` row of kind `ssh_git`, not as a
second XID and not as an additional Sign key on the C2 document.
Honor's `owner_id` SHALL equal that XID as the public OIDC `sub`
...
#### Scenario: Friend identity survives a new Honor VM

- GIVEN a stored C2 identikey for the friend
- WHEN a Honor VM is provisioned with a fresh A3 assertion
- THEN the friend's XID is unchanged
- AND a `credentials` row of kind `ssh_git` exists for that XID
- AND Honor's `owner_id` equals that XID
...
- THEN it is not a Honor being
- AND no `ssh_git` credentials row is inserted

#### Scenario: Duke-owned spawn is not Honor

- GIVEN Duke's OIDC `sub` is not the friend's XID
- WHEN Duke spawns a VM under his own token
- THEN `owner_id` is Duke's `sub`
- AND the VM is not a Honor being of the friend

### Requirement: Browser passkey opens /term

Login to Honor `/term` from a browser SHALL complete WebAuthn
assertion at `https://auth.identikey.me` (RP ID
...
- GIVEN a friend with a registered passkey on the C2 identikey
  and a Honor VM whose `owner_id` is that XID
...
The Honor VM SHALL sign git commits with an SSH key whose private
...
Honor SHALL run the storefront in Vite dev mode reachable at
`https://<ticket>.vm.worldtree.network`. That HTTP surface SHALL
NOT require passkey (the passkey gate is `/term`). The frontend
SHALL use `https://api.hypersigil.world` as the Medusa backend.
Prod Medusa CORS SHALL list that exact ticket origin and SHALL
NOT use a `*.vm.worldtree.network` wildcard. Honor SHALL clone
`ubuntu-24.04` and MAY persist a per-friend snapshot `honor-<xid>`
(not a catalog entry; ADR 0009). Honor SHALL NOT add a declared
`@base/` name.

#### Scenario: Preview talks to prod API

- GIVEN Honor Vite is up and the ticket URL loads
...
`honor-<xid>` SHALL set `preserve_iroh_key: true` so the ticket
...
- GIVEN Honor was snapshotted as `honor-<xid>` with an Iroh key
