...
- [ ] ADR 0007 filed at `docs/decisions/0007-host-sidecar-redis.md`
...
- [ ] Living delta `openspec/changes/add-host-sidecar-redis/specs/host-redis/spec.md`
      (ADDED). Fold later copies it to `openspec/specs/host-redis/spec.md`.
- [ ] Advise accept (reader **not** Grok — author family). Banner stays
      ACTIVE BUILD until fold.

## Install (`mjolnir-g193.2`) — blocked on .1

- [ ] Distro `redis-server` on the host; disable stock `redis-server.service`
- [ ] `systemd/mjolnir-redis.service` + `redis.conf` under `/etc/mjolnir`
...
- [ ] `scripts/install-redis.sh`: fail closed if dummy `10.200.0.1/32`
      missing; never bind `0.0.0.0`; restart unit only if
      conf/unit/binary/password hash changed; does not restart Elixir
- [ ] Ride `scripts/deploy.sh` / `just deploy`. No `just deploy-redis`
- [ ] Bootstrap UFW/iptables INPUT `10.192.0.0/10` → `10.200.0.1:6379`
      (same helper shape as 5432/7222)
- [ ] `mix mjolnir.redis.ensure --slug hypersigil-api` merges `REDIS_URL`
      into `/var/lib/mjolnir/deploy/secrets/hypersigil-api.json` without
      dropping `DATABASE_URL`
- [ ] Catalog: `docs/guide/host-sidecars.md` row, README sidecar table,
      CLAUDE.md one-liner. Not `vm.json`
- [ ] Tests / live proof: `ss` listen `10.200.0.1:6379` only; unauth
...
- [ ] `scripts/backup-redis-b2.sh`: unix-socket `BGREWRITEAOF`, wait,
      rclone copy data dir → `b2:mimir-backups/mjolnir-redis/<hostname>/`
- [ ] `scripts/systemd/mjolnir-redis-backup.{service,timer}` (daily,
      RandomizedDelaySec, same shape as pg-tenants)
- [ ] Runbook `docs/runbooks/host-redis.md`: restore (stop, replace
      dir, start) + exit `redis-cli --rdb` over `REDIS_URL`
- [ ] Restore exercised once on the live host or a throwaway data dir
