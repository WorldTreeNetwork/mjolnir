...
- [x] Parse `[site].fallback` in `mjolnir.toml` (`index.html` |
      `404.html` | `false`); persist as sibling of materialized
      `current`
- [x] Gateway miss path: SPA 200 / snapshot-or-config 404.html /
      empty 404 / park body at 404, original Host
- [x] Existing snapshot files still 200; live miss never 307s to park
- [x] Tests: Vite-style `GET /about` → 200 index.html; Hugo
      `404.html` → 404 body; `fallback = false` empty 404; park
      file in place when no policy and no snapshot 404.html
- [x] Docs: `mjolnir.toml` `[site].fallback` and the park default

## Owed from architecture advise — 2026-09-21

- [x] R1: Pin TOML discovery, site-only parsing, authenticated policy transport,
...
- [x] R2: Pin park cache scope and refresh/recovery on late publish, republish,
...
- [x] R3: Pin missing/unreadable explicit targets and invalid policy handling;
